...
class API extends REST {
    ...
    public function processApi() {
        ...
        if ((int) method_exists($this, $func) > 0) {
            ...
            if (isset($operator) || ( isset($sdate) || isset($edate) ) ) {
                $this->$func($operator, $sdate , $edate);
            } else {
                $this->$func();
            }
        } else
    ...
    private function mData($para = '', $sdate = '',$edate = '') {
        ...
        if (!empty($para)) {
            $query = "SELECT distinct c.latitude, c.longitude, count(*) as count FROM calldrop c where c.operator = '$para' group by c.latitude, c.longitude ";
        }  else if (!empty ($sdate) && !empty($edate)) {
            $query = "SELECT  distinct c.latitude, c.longitude, count(*) as count FROM calldrop c where c.date >= '$sdate' AND c.date <=  '$edate' group by c.latitude, c.longitude ";
        }  else if (!empty($sdate)) {
            $query = "SELECT distinct c.latitude, c.longitude, count(*) as count FROM calldrop c where c.date like '$sdate%' ";
        } else if (!empty($edate)) {
            $query = "SELECT distinct c.latitude, c.longitude, count(*) as count FROM calldrop c where c.date like '$edate%' ";
        }  else {
            $query = "SELECT distinct c.latitude, c.longitude, count(*) as count FROM calldrop c group by c.latitude, c.longitude ";
        }
    ...
    private function insertData()
    {
        ...
        if (!empty($customer)) {            
            foreach ($customer as $mdata) {
                ...
                foreach ($column_names as $desired_key) { // Check the customer received. If blank insert blank into the array.
                    if (!in_array($desired_key, $keys)) {
                        ...
                        $$desired_key = $mdata[$desired_key];
                    }
                    $columns = $columns . $desired_key . ',';
                    $values = $values . "'" . $$desired_key . "',";
                }
                $query = "INSERT INTO calldrop(" . trim($columns, ',') . ") VALUES(" . trim($values, ',') . ")";
                if (!empty($mdata)) {
            ...
            $success = array('status' => "Success", 'No of Records Inserted' => count($r)); //, "msg" => "Data Inserted Successfully.", "data" => $customer
